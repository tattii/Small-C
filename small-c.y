...
rule
  ...
  function_definition: type_specifier function_declarator compound_statement { result = [:function, val[0], val[1], val[2]] }
  ...
  statement: ';'                                                                        { result = [:skip] }
           ...
           | FOR '(' expression_opt ';' expression_opt ';' expression_opt ')' statement { result = [:for, val[2], val[4], val[6], val[8]]}
  ...
  postfix_expr: primary_expr 
              ...
              | IDENT '(' argument_expression_list_opt ')' { reuslt = [:call, val[0], val[2]] }

  primary_expr: IDENT
              | NUMBER
              | '(' expression ')' { result = val[1] }

  argument_expression_list_opt: /* optional */
                              | argument_expression_list

  argument_expression_list: assign_expr                              { result = [:arg, val[0]] }
                          | argument_expression_list ',' assign_expr { result.push val[2] }
